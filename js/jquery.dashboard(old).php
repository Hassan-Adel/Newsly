...
! function($) {
    ...
    Dashboard.prototype.createPlotGraph = function(selector, data1, labels, colors, borderColor, bgColor) {
            ...
            $.plot($(selector), [{
                ...
                tooltip: true,
                tooltipOpts: {
                    content: '%s: <?php echo 'Hassan'; ?> have is %y news',
                    shifts: {
                        x: -60,
                        y: 25
                    },
...
$result = mysqli_query($con,"SELECT * FROM `users` Where `PrivilegeAdd`= '1'");
$count= 0;
$num= mysqli_num_rows($result); 
while($row = mysqli_fetch_array($result)){
    $result1 = mysqli_query($con," SELECT * FROM `log` WHERE `Action` = 'Add News' and `UserID`='$row[UserID]' " );
                ?>
                ["<?php echo $row['Username']; /*$count;*/ $count+=1;?>" ,<?php echo mysqli_num_rows($result1);  ?>]
